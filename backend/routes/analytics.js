...
router.get('/activity/today', verifyAdmin, async (req, res) => {
  try {
    console.log("📅 Fetching today's activity from login_events...");
    
    // Use login_events table directly since trigger is disabled
    ...
    console.log(`Found ${data.rows.length} activities for today`);
    res.json(data.rows);
  } catch (err) {
...
router.get('/activity/week', verifyAdmin, async (req, res) => {
  try {
    console.log("📊 Fetching weekly activity from login_events...");
    ...
    console.log(`Found ${data.rows.length} activities for the week`);
    res.json(data.rows);
  } catch (err) {
...
// Get currently active users (since no logout tracking, show recent logins)
router.get('/activity/active', verifyAdmin, async (req, res) => {
  try {
    console.log("🟢 Fetching active users from login_events...");
    
    // Since there's no logout tracking, consider users active if they logged in within last 2 hours
    const data = await pool.query(`
    ...
    console.log(`Found ${data.rows.length} active users`);
    res.json(data.rows);
  } catch (err) {
...
router.get('/activity/summary', verifyAdmin, async (req, res) => {
  try {
    console.log("📈 Fetching activity summary from login_events...");
    
    const data = await pool.query(`
      SELECT 
        DATE(login_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Dhaka') AS day, 
        role, 
        COUNT(*) AS logins,
        COUNT(DISTINCT user_id) AS unique_users
      FROM login_events
      WHERE login_time >= NOW() - INTERVAL '30 days'
      GROUP BY DATE(login_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Dhaka'), role
      ORDER BY day DESC, role
    `);
    
    console.log(`Found ${data.rows.length} summary records`);
    res.json(data.rows);
  } catch (err) {
...
router.get('/activity/all', verifyAdmin, async (req, res) => {
  try {
    console.log("📋 Fetching all activity from login_events...");
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM login_events');
    const total = parseInt(countResult.rows[0].count);

    console.log(`Found ${data.rows.length} activities (page ${page})`);
    res.json({
      data: data.rows,
      pagination: {
        page,
...
router.get('/activity/stats', verifyAdmin, async (req, res) => {
  try {
    console.log("📊 Fetching activity statistics from login_events...");
    ...
    const stats = await pool.query(`
      SELECT 
        ...
        COUNT(CASE WHEN login_time >= NOW() - INTERVAL '2 hours' THEN 1 END) as currently_active,
        COUNT(CASE WHEN DATE(login_time AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Dhaka') = CURRENT_DATE THEN 1 END) as today_logins,
        COUNT(CASE WHEN login_time >= NOW() - INTERVAL '7 days' THEN 1 END) as week_logins
      FROM login_events
    `);

    console.log("📊 Activity stats:", stats.rows[0]);
