...
exports.createExam = async (req, res) => {
  try {
    const { class: examClass, title, year } = req.body;
    ...
    const result = await pool.query(
      `INSERT INTO exams (class, title, year)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [examClass, title, year]
    );
...
exports.addExamSet = async (req, res) => {
  try {
    ...
    const { set_name, answer_key } = req.body;

    if (!set_name || !answer_key) {
      return res.status(400).json({ message: "Missing required fields" });
    }

 const result = await pool.query(
  `INSERT INTO exam_sets (exam_id, set_name, answer_key)
   VALUES ($1, $2, $3)
   ON CONFLICT (exam_id, set_name) 
   DO UPDATE SET answer_key = EXCLUDED.answer_key, created_at = NOW()
   RETURNING *`,
  [examId, set_name, answer_key]
);

    res.status(201).json({
      message: "Exam set added successfully",
...
exports.getAllExams = async (_req, res) => {
  try {
    const result = await pool.query(
      `SELECT id, class, title, year, created_at FROM exams ORDER BY year DESC, created_at DESC`
    );
...
exports.editExamSet = async (req, res) => {
  try {
    ...
    if (set_name !== undefined) { fields.push(`set_name = $${idx++}`); values.push(set_name); }
    if (answer_key !== undefined) { fields.push(`answer_key = $${idx++}`); values.push(answer_key); }
    ...
    const result = await pool.query(
      `UPDATE exam_sets SET ${fields.join(", ")} WHERE id = $${idx++} AND exam_id = $${idx} RETURNING *`,
      values
    );

    if (result.rowCount === 0) return res.status(404).json({ message: "Exam set not found" });
    res.json({ message: "Exam set updated", examSet: result.rows[0] });
  } catch (err) {
    if (err.code === "23505") {
